...
class RoPE(torch.nn.Module):
    ...
    def forward(
        ...
        ), "Input sequence length exceeds maximum sequence length"
        assert (
            x.shape[-1] == self.cos_cached.shape[1] * 2
        ), "Input feature dimension must be twice d_k"
        ...
        x_rotated = torch.stack((-x2, x1), dim=-1).reshape_as(
            x
        )  # stack elements in last dim as pairs
